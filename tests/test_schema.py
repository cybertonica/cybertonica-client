from CybertonicaAPI.schema import Schema
import os
import sys
import json
import unittest
from unittest.mock import patch, PropertyMock, Mock, MagicMock

sys.path.append(os.getcwd())


class TestInitSchemaClass(unittest.TestCase):

	def setUp(self):
		self.schema = Schema(PropertyMock(
			url='test_url',
			team='test_team',
			token='old_value',
			verify=True,
			r=Mock(return_value=(200, {'token': '123'}))
		))

	def test_client_object_creation(self):
		self.assertIsInstance(self.schema, Schema)

		self.assertTrue("get_all" in dir(self.schema))
		self.assertTrue("get_by_id" in dir(self.schema))
		self.assertTrue("create" in dir(self.schema))
		self.assertTrue("update" in dir(self.schema))
		self.assertTrue("delete" in dir(self.schema))

	def test_attributes_inside_auth_object(self):
		self.assertTrue(hasattr(self.schema, 'root'))

	def test_types_of_fields_inside_auth_object(self):
		self.assertIsInstance(self.schema.root, object)

class TestGetAllMethod(unittest.TestCase):

	def setUp(self):
		self.schema = Schema(PropertyMock(
			url='test_url',
			team='test_team',
			signature='test_signature',
			token='old_value',
			verify=True,
			r=Mock(return_value=(200, {'token': '123'}))
		))

	def test_get_all_request(self):
		self.schema.get_all()
		self.schema.root.r.assert_called_with(
			'GET',
			f'{self.schema.root.url}/api/v1.2/schemas',
			body=None,
			headers=None,
			verify=self.schema.root.verify
		)

class TestCreateMethod(unittest.TestCase):

	def setUp(self):
		self.schema = Schema(PropertyMock(
			url='test_url',
			team='test_team',
			token='old_value',
			verify=True,
			r=Mock(return_value=(200, {'token': '123'}))
		))
		self.data = {'a': 1, 'b': 2}

	def test_create_request(self):
		self.schema.create(self.data)
		self.schema.root.r.assert_called_with(
			'POST',
			f'{self.schema.root.url}/api/v1.2/schemas',
			json.dumps(self.data),
			headers=None,
			verify=self.schema.root.verify
		)

	def test_create_with_incorrect_data_type(self):
		with self.assertRaisesRegex(AssertionError, 'The data type must be a dictionary'):
			self.schema.create(123)
	
	def test_create_with_empty_dict(self):
		with self.assertRaisesRegex(AssertionError, 'Schema data must not be an empty dictionary'):
			self.schema.create({})

class TestUpdateMethod(unittest.TestCase):

	def setUp(self):
		self.schema = Schema(PropertyMock(
			url='test_url',
			team='test_team',
			token='old_value',
			verify=True,
			r=Mock(return_value=(200, {'token': '123'}))
		))
		self.id = 'test_id'
		self.data = {'a': 1, 'b': 2}

	def test_update_request(self):
		self.schema.update(self.id, self.data)
		self.schema.root.r.assert_called_with(
			'PUT',
			f'{self.schema.root.url}/api/v1.2/schemas/{self.id}',
			json.dumps(self.data),
			headers=None,
			verify=self.schema.root.verify
		)

	def test_update_with_incorrect_id_type(self):
		with self.assertRaisesRegex(AssertionError, 'The ID must be a string'):
			self.schema.update(123,self.data)
	
	def test_update_with_incorrect_data_type(self):
		with self.assertRaisesRegex(AssertionError, 'The data type must be a dictionary'):
			self.schema.update(self.id,123)
	
	def test_update_with_empty_id_string(self):
		with self.assertRaisesRegex(AssertionError, 'The ID must not be an empty string'):
			self.schema.update('',self.data)
	
	def test_update_with_empty_data_dict(self):
		with self.assertRaisesRegex(AssertionError, 'Schema data must not be an empty dictionary'):
			self.schema.update(self.id,{})

class TestDeleteMethod(unittest.TestCase):

	def setUp(self):
		self.schema = Schema(PropertyMock(
			url='test_url',
			team='test_team',
			token='old_value',
			verify=True,
			r=Mock(return_value=(200, {'token': '123'}))
		))
		self.id = 'test_id'

	def test_delete_request(self):
		self.schema.delete(self.id)
		self.schema.root.r.assert_called_with(
			'DELETE',
			f'{self.schema.root.url}/api/v1.2/schemas/{self.id}',
			body=None,
			headers=None,
			verify=self.schema.root.verify
		)

	def test_update_with_incorrect_id_type(self):
		with self.assertRaisesRegex(AssertionError, 'The ID must be a string'):
			self.schema.delete(123)
	
	def test_update_with_empty_id_string(self):
		with self.assertRaisesRegex(AssertionError, 'The ID must not be an empty string'):
			self.schema.delete('')